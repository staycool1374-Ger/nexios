...
**Build:** v0.3.10-dev | **Last Release:** v0.3.9 | **Completed milestones:** see `ROADMAP_done.md` (v0.2.x — v0.3.11)
...
corruption with no diagnostic).  **All items below fixed 2026-08-10 (v0.3.10).**

#### (A) CRITICAL — unchecked alloc return → NULL/0 deref (fix first)

- [x] **A1 — `init_kstack_window` (task.cpp:344, 355, 366):**
...
- [x] **A2 — `Scheduler::init` (scheduler.cpp:421-423):**
...
- [x] **A3 — `Scheduler::reap` (scheduler.cpp:1448-1458):**
      ...
      valid.
- [x] **A4 — `map_page_in_pml4` RV64 (vmm.cpp:478, 492):**
...
- [x] **A5 — `map_page` RV64 (vmm.cpp:222, 242, 253):** same unchecked
      `get_table` pattern in the kernel-mapping path.  Fix as A4.
- [x] **A6 — `map_page` x86_64 (vmm.cpp:311, 327):**
...
- [x] **B1 (HIGH) — `IPC::send` (ipc.cpp:240):**
      ...
      queued handle.
- [x] **B2 — `exec_into_current` (elf.cpp:489-491):**
...
- [x] **B3 — `create_user` (task.cpp:696-708):**
...
- [x] **B4 — `virtio_net.cpp` probe (139, 150, 159, 174):**
...
- [x] **B5 — `AhciDriver::port_init` (ahci.cpp:198-203):**
...
- [x] **B6 — ENSURE-on-OOM → panic (vmm.cpp:139/227/289, mempool.cpp:54,
      ...
      of `ENSURE`, and let callers handle it (after A4-A6).  Note: the vmm.cpp
      split paths now return nullptr/early-return; mempool.cpp:54-55 and
      buffer_pool.cpp:173-174 RETAIN ENSURE by design (one-shot boot-only
      inits; documented deviation).
- [x] **B7 — `register_driver` (driver.cpp:40-45):** stores a nullptr in
...
- [x] **C1 — `TaskControlBlock::cleanup` (task.cpp:1275):**
...
- [x] **C2 — `exec_into_current` (elf.cpp:563-567):** identical latent pattern —
      `PMM::free_page(old_pml4)` not gated on `old_shared`.  Fix as C1.
- [x] **C3 — `BufferPool::alloc_page` (buffer_pool.cpp:190-195):**
      ...
      (see C4).  **Drift note: already fixed in v0.3.11 (buffer_pool.cpp:435-440,
      454) — capture_state/restore_state now include `pool_pages_[]`; verified
      2026-08-10, no v0.3.10 code change.**
- [x] **C4 — `BufferPool::free_page` (buffer_pool.cpp:209-219):** when the pool
      ...
      **Implemented at the pop side instead: `alloc_page` scrubs the slot
      (`pool_pages_[idx] = 0`) — symmetric with free_page's push, same
      stale-slot elimination, verified by `buffer_pool`/
      `buffer_pool_deterministic`/`memory` gates (2026-08-10).**
...
**Acceptance criteria (DONE — verified 2026-08-10):**
- [x] A1-A6, B1-B7, C1-C4 fixed (each verified by build + class gate).
- [x] B2-B5 leak paths closed (no new ResourceTracker deltas in `elf`/`process`/
  `driver`/`vfs` classes).
- [x] `make build` clean (check-style Errors: 0), `selftest` 132/132.
- [x] `test-history.txt` rows appended for every class touched.
- [x] Final gate: `make execute-test x86_64 debug all` → 825/825 (2026-08-10;
  suite grew 817 → 825 since the 2026-08-08 baseline).
