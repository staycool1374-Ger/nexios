...
## Active Summary
### Objective
- **IN PROGRESS (2026-08-31):** Issue #91 "ISR_NOERR misclassification — #VE(21)/#HV(28) push error codes" (severity:S1). Fix: isr_stubs.asm 21/28 → ISR_ERR (real CPU error code, was consumed as RAX → misaligned iretq on hosts that raise them) + %assign frame-depth audit + exported __isr_vectors_err_mask; kernel.cpp reserved-vector fail-stop branch (15/22-26/29/31 → panic, spec §3.4) + weak exception_dispatch_probe/reserved_exception_hook; exc_table class (3 tests, now in `all` gate); ud-probe/de-probe (inline-asm #UD/#DE). Gates: exc_table 3/3, debug all **981/981** (996 reg), release all 85/85, release selftest 85/85. SIL 3 APPROVED (2 reports). Commit `1b45a37b` (closes #91). Count fixes: all 990→996 (memory_pmm 5→8 #100 + exc_table), safe/selftest 132→133.
- **DONE (2026-08-31):** Issue #100 aarch64 bring-up COMPLETE — daemon-ready handshake fixed (syscall x8 ABI, scheduler frame validation, kernel-task SPSR 0x345, page-table HHDM + 48-bit mask, ECAM 0x3f000000, boot tables 0x40020000, PL031 RTC). arch_aarch64 **22/22**, debug all **978/978**, release all **85/85**. Commit `4b2bef37` (closes #100).
